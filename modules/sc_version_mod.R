...
sc_version_server <- function(input, output, session) {
...
# Render Plot -------------------------------------------------------------
  observe({
    ...
    output$Plot <- renderPlotly({
      fig <-
        if (input$display != "Line chart") {
          bar.plot(sc_version_data, input$Selections, "% of National", "%")
        } else {
          line.plot(sc_version_data, input$Selections, "% of National", "%")
        }
    })
  })
