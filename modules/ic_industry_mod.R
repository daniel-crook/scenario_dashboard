                   ...
                   column(
                     ...
                     radioGroupButtons(
                       ...
                       choices = c(
                         "Industry Share Line",
                         "Industry Share Bar",
                         "Cont. to Gross Output"
                       ),
                       selected = "Industry Share Line",
                       justified = TRUE,
                       status = "primary"
                     )
                   ),
...
ic_industry_server <- function(id, data) {
  moduleServer(id, function(input, output, session) {
    ...
    #   version_list <-
    #     data.frame(
    #       ATTRIBUTE = unique(ic_data$ATTRIBUTE]),
    #       STATE = input$State,
    #       SCENARIO_VALUE = input$Scenario,
    #       RELEASE_VERSION = input$Version,
    #       Series_ID = unique(ic_data$Series_ID)
    #     )  %>%
    #     add.var.col(.)
    #
    #   updatePrettyCheckboxGroup(
    ...
    # Render Plot -------------------------------------------------------------
    observe({
      if (str_after_first(input$Selections[1], ", ") == paste(input$State, input$Scenario, input$Version, sep = ", ")) {
        ...
        if (input$display == "Cont. to Gross Output") {
          ...
          if (input$display != "Industry Share Line") {
            ic_industry_data <-
              trail_avg(ic_industry_data, bar_dates[2] - bar_dates[1]) %>%
              mutate(., value = round(value, 2)) %>%
              filter(., Dates %in% bar_dates)
            ic_industry_data$Dates <-
              as.factor(ic_industry_data$Dates)
          }
          ic_industry_data <-
            spread(ic_industry_data, variable, value)
        }
        
        output$Plot <- renderPlotly({
          fig <-
            if (input$display == "Industry Share Bar") {
              {
                fig <- plot_ly(
                  ic_industry_data,
                  x = ~ Dates,
                ...
                if (length(input$Selections) >= 2) {
                  for (i in 2:length(input$Selections)) {
                    fig <-
                      fig %>% add_trace(
                        y = ic_industry_data[[input$Selections[i]]],
            ...
            } else if (input$display == "Industry Share Line") {
              {
                fig <- plot_ly(
                  ic_industry_data,
                  x = ~ Dates,
