               ...
               wellPanel(
                 ...
                 fluidRow(column(
                   ...
                   prettyCheckboxGroup(
                     ns("Selections"),
                     label = NULL,
                     choices = sort(unique(gem_data$sv[gem_data$Attribute == "Gross Domestic Product"])),
                     selected = sort(unique(gem_data$sv[gem_data$Attribute == "Gross Domestic Product"]))[1:3],
                     shape = "round",
                     outline = TRUE,
                     status = "primary"
                   )
...
gc_scenario_server <- function(id, gem_data) {
  moduleServer(id, function(input, output, session) {
    ...
    global_gc_s <- reactiveValues(checked = NULL)
    variables_gc_s <- reactiveValues(variable = NULL)
    
    observe({
      gem_data_f <-
        filter(gem_data, gem_data$Attribute == input$Attribute)
      
      possible_gc_s <-
        unique(gem_data_f[c("Scenario", "Release")]) %>% separate(Release, c('Release_Date', 'Version'))
      
      possible_gc_s$Release_Date <-
        parse_date_time(possible_gc_s$Release_Date, "my")
      
      possible_gc_s <-
        arrange(possible_gc_s,
                Scenario,
                desc(Release_Date),
                desc(Version)) %>%
        transmute(Scenario,
        ...
        transmute(., sv = paste(Scenario, Release, sep = ", "))
      
      global_gc_s$checked <- possible_gc_s$sv[(possible_gc_s$sv %in% input$Selections)]
      
      if (length(gem_data$variable[gem_data$Attribute == input$Attribute &
                                   gem_data$sv %in% input$Selections]) >= 1) {
        checked_list_gc_s <-
          data.frame(Attribute = input$Attribute,
                     sv = possible_gc_s$sv[(possible_gc_s$sv %in% input$Selections)]) %>%
          mutate(variable = paste(Attribute, sv, sep = ", "))
        
        variables_gc_s$variable <- checked_list_gc_s$variable
      }
      ...
      if (input$Attribute %in% gem_data$Attribute) {
        ...
        sv_list <-
          ...
          transmute(Scenario,
                    Release = paste(format(Release_Date, format = "%b%y"), Version, sep = " ")) %>%
          distinct(Scenario, .keep_all = TRUE) %>%
          mutate(sv = paste(Scenario, Release, sep = ", "))
        ...
        updatePrettyCheckboxGroup(
          ...
          sv_list$sv,
          selected =
            if (length(sv_list$sv[(sv_list$sv %in% global_gc_s$checked)]) >= 1) {
              sv_list$sv[(sv_list$sv %in% global_gc_s$checked)]
            } else {
              sv_list$sv[1:3]
            },
          prettyOptions = list(
    ...
    observe({
      if (length(input$Selections) >= 1) {
        gc_scenario_data <- filter(gem_data,
                                   variable %in% variables_gc_s$variable) %>%
          transmute(., Dates,
                    variable,
        ...
        gc_scenario_data <-
          gc_scenario_data[, append("Dates", variables_gc_s$variable)]
        ...
        output$Plot <- renderPlotly({
          fig  <-  plot_ly(
            ...
            name = str_after_first(variables_gc_s$variable[1], ", "),
            type = 'scatter',
            mode = 'lines',
            ...
            add_annotations(
              x = min(gc_scenario_data$Dates[!is.na(gc_scenario_data[[variables_gc_s$variable[1]]])]),
              y = 1.035,
          ...
          if (length(gc_scenario_input) >= 2) {
            for (i in 2:length(gc_scenario_input)) {
              fig <- fig %>% add_trace(
                ...
                name = str_after_first(variables_gc_s$variable[i], ", "),
                hoverlabel = list(namelength = -1)
              )
            }
          }
    ...
    observe({
      if (length(input$Selections) >= 1) {
        ...
        output$Table <- renderTable({
          ...
          if (unique(gem_data$Aggregation[gem_data$variable %in% variables_gc_s$variable]) == "Sum") {
            gc_scenario_table_data <- trail_sum(gc_scenario_table_data)
          } else if (unique(gem_data$Aggregation[gem_data$variable %in% variables_gc_s$variable]) == "Avg") {
          ...
          vc_version_table_data1 <- gc_scenario_table_data %>%
            ...
            transmute(Period = paste0("(", lag(Dates), " - ", Dates, ")"),
                      variable, value) %>%
            filter(
              Period != paste0("(NA - ", min(cagr_dates), ")") &
                Period != paste0("(",
                                 max(cagr_dates),
                                 " - ",
                                 min(cagr_dates),
          ...
          vc_version_table_data1 <-
            vc_version_table_data1[, append("Period", variables_gc_s$variable)]
