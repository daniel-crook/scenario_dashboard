...
{
  fp <-
    "S:/BIS Consultancy/Product Development/Scenarios_Dashboard/Data Processing/"
  ...
  AID_db_folder <- "C:/AID/AEMO/2022/03_AID/"
...
# 2.1 Parameters that don't require adjusting -----------------------------
{
  ...
  AID_db_filename <- substr(AID_db_filepath,
    stringi::stri_locate_last_fixed(AID_db_filepath, "/") + 1,
    nchar(AID_db_filepath)
  )
  
  #Check file type
  if(substr(AID_db_filename,nchar(AID_db_filename)-2,nchar(AID_db_filename)) != ".db"){
  ...
  Scenario_name <- toupper(substr(AID_db_filename, 1, stringi::stri_locate_first_fixed(AID_db_filename, "_") - 1
  ))
  ...
  if (as.numeric(str_count(AID_db_filename,"AID_")) == 1 & as.numeric(str_count(AID_db_filename,"_")) == 3) {
  ...
  if(as.numeric(str_count(AID_db_filename,"_V")) == 1) {
    version <- substr(AID_db_filename,
...
rds_files <- list.files(paste0(fp,"RDS Files/"))

if(paste0(Scenario_name, "_",release_version,"_Output_Data.rds") %in% rds_files) {
  print(paste0(Scenario_name, "_",release_version,"_Output_Data.rds RDS file already exists"))
} else {
...
{
  list.of.indicators <-
    openxlsx::read.xlsx(file.path(fp, "Input Files/", indicator_list_file_name)) %>%
    dplyr::rename(Sector = Division) # List of Indicators to extract from AID
  
  Rebasing_Coeff <-
    openxlsx::read.xlsx(file.path(fp, "Input Files/", "RebasingCoefficients.xlsx")) %>% 
  ...
  meta_data_df <-
    openxlsx::read.xlsx(file.path(fp, "Input Files/", meta_data_file_name))
}
...
# 5.0 Get AID Data --------------------------------------------------------
{
  ...
  saveRDS(AID_db[[1]], file = file.path(fp, paste0("AID_db_", Scenario_name, ".rds")))
  ...
  AID_db_Total <- AID_db_States %>% dplyr::bind_rows(AID_db_Buildings)
}
...
# 8.0 Extending Forecasts past FY50 and adding hist/fc column -------------
{
    ...
    Date_Df <-
      data.frame(YYYYQ = seq(as.Date("1999/3/1"), by = "quarter", length.out = (as.numeric(end_yr) - as.numeric(st_yr))*4+2)) %>%
      dplyr::left_join(
    ...
    Df <-
      Df %>% select(length(Df), 1:{length(Df)-1}) %>% tidyr::gather(key = Series_Name, value = value, -YYYYQ) %>%
...
{
    Output_Data <-
      Output_Data %>% dplyr::left_join(list.of.indicators %>%
...
# 11.0 Extract, Wrangle & Aggregate Macro Data to FY ----------------------
{
    ...
    Date_Df <-
      data.frame(YYYYQ = seq(as.Date("1999/3/1"), by = "quarter", length.out = 218)) %>%
      dplyr::left_join(
    ...
    Output_Data_Macro <- Output_Data_Macro %>%
      ...
      dplyr::left_join(list.of.indicators %>%
                         dplyr::select(Series_ID, Aggregation_Rule) %>%
                         unique(),
                       by = "Series_ID") %>%
      dplyr::select(-Year, -Quarter, -Aggregation_Rule) %>%
      dplyr::group_by(Division, Source, Extract_Date, Region, Series_ID, Scenario, FY) %>%
      ...
      dplyr::mutate(FORECAST_FLAG = "",
                    TEMPORAL = "Annual") %>%
      dplyr::arrange(RELEASE_VERSION, STATE, ATTRIBUTE, MPE_DATE, SCENARIO_VALUE) %>%
...
# 12.0 Combine Macro & State Data, and Write Output -----------------------
{
    ...
    Tot_Output_Data_FY <- Tot_Output_Data_FY %>%
      ...
      dplyr::mutate(test_forecast_flag = ifelse(
        MPE_DATE >= st_yr &
    ...
    Tot_Output_Data_FY[is.na(Tot_Output_Data_FY)] <- "" #Convert NA to blanks
    ...
    saveRDS(Tot_Output_Data_FY, file = file.path(fp,"RDS Files/",paste0(Scenario_name, "_",release_version,"_Output_Data.rds")))
